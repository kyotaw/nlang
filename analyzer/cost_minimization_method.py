...
class CostMinimizationMethod(object):
    def __init__(self, vocabulary, connection):
        self.__vocab = vocabulary
        self.__conn = connection
        self.__granularity = -1

    def extract_paths(self, data_stream):
        bos_node = {'data':self.__vocab.get_bos(), 'total_cost':0, 'prev':None, 'start_index':-1}
        start_node_list = {}
        end_node_list = {}
        ...
        for i in range(0, length + 1):
            ...
            if i < length:
                chunks = self.__vocab.extract_vocabulary(self.__get_data_grain(data_stream, i))
            else:
                chunks = [self.__vocab.get_eos()]
        ...
        for i in range(length+1):
            ...
            for right_node in start_nodes:
                ...
                for left_node in end_nodes:
                    occurr_cost = occurr_cost_func(right_node) if occurr_cost_func else self.__occurr_cost(right_node)
                    conn_cost = self.__conn_cost(left_node, right_node)
    ...
    def __occurr_cost(self, node):
        return node['data'].cost
    
    def __conn_cost(self, left_node, right_node):
        return self.__conn.cost(left_node['data'].get_hook(), right_node['data'].get_hook())
    ...
    def __get_data_grain(self, data_stream, start_index):
